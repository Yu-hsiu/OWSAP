...
>
> 📌 **目前進度**:本 repo 已包含 **API1 ~ API5**(各含 vulnerable / secure / 攻擊
> 腳本 / 一鍵展示);API6 ~ API10 為規劃中的路線圖(見下方進度表)。
...
- **API1–API5 已完成**(BOLA、BrokenAuth、BOPLA、ResourceConsumption、BFLA,本機實跑通過)。其餘五項待逐項展開。
...
| API6 業務流程濫用 | `API6-BusinessFlows/` | ☐ | ☐ | ☐ | ☐ | 未開始 |
| API7 SSRF | `API7-SSRF/` | ☐ | ☐ | ☐ | ☐ | 未開始 |
| API8 設定錯誤 | `API8-Misconfiguration/` | ☐ | ☐ | ☐ | ☐ | 未開始 |
| API9 庫存管理 | `API9-InventoryMgmt/` | ☐ | ☐ | ☐ | ☐ | 未開始 |
| API10 不安全取用 | `API10-UnsafeConsumption/` | ☐ | ☐ | ☐ | ☐ | 未開始 |

---

<sub>本規劃由 Claude Code 依 freeCodeCamp 影片內容與職涯規劃工作流程三擬定。
專案設計為可持續更新,小步前進、每步可驗證。建立日期 2026-07-16。</sub>
